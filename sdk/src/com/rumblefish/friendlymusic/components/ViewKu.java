...
public class ViewKu extends View
{
    ...
    public ViewKu(Context context, int i, int j, int k, int l)
    {
        super(context);
        setFrame(i, j, k, l);
    }

    public ViewKu(Context context, AttributeSet attributeset)
    {
        super(context, attributeset);
    }

    public ViewKu(Context context, AttributeSet attributeset, int i)
    {
        super(context, attributeset, i);
    }

    public ViewKu(Context context, FrameKu frameku)
    {
        super(context);
        setFrame(frameku);
    }
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        {
            tempPaint = new Paint();
            tempPaint.setStyle(android.graphics.Paint.Style.STROKE);
            tempPaint.setStrokeWidth(10 * Utils.screenScaleX());
            tempPaint.setStrokeCap(Paint.Cap.ROUND);
            tempPaint.setStrokeJoin(Paint.Join.ROUND);
            tempPaint.setAntiAlias(true);
        }
        tempPaint.setColor(_paintColor);
        if(_path != null)
            canvas.drawPath(_path, tempPaint);
    }
    ...
    {
        ViewContainerKu viewcontainerku = parent();
        if(viewcontainerku != null)
            ((ViewContainerKu)viewcontainerku).removeView(this);
    }
